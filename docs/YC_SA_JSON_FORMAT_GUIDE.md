...
2. **ВНИМАНИЕ**: При копировании токена убедитесь, что не захватываете лишние символы:
   - Копируйте только сам токен (длинная строка без пробелов)
   - Не копируйте символы `$`, `#` или другие лишние символы
   - Токен должен начинаться с `t1.` и не содержать пробелов

3. Добавьте токен в GitHub Secrets как `YC_IAM_TOKEN`

4. Используйте workflow с IAM токеном

**Проверка правильности токена:**
```bash
# Получите токен и проверьте его
TOKEN=$(yc iam create-token)
echo "Длина токена: ${#TOKEN}"
echo "Начинается с: ${TOKEN:0:3}"
echo "Содержит пробелы: $(echo "$TOKEN" | grep -q " " && echo "ДА" || echo "НЕТ")"
```
...
### Решение проблемы "Token contains an illegal character ' '"
Если получаете ошибку с пробелом в токене:

1. **Удалите текущий секрет** `YC_IAM_TOKEN` из GitHub
2. **Получите новый токен**:
```bash
yc iam create-token | tr -d '[:space:]'
```

3. **Проверьте токен перед добавлением**:
```bash
TOKEN=$(yc iam create-token | tr -d '[:space:]')
echo "$TOKEN" | od -c  # Покажет все символы токена
```

4. **Добавьте очищенный токен** в GitHub Secrets
